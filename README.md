...
View the Interactive [Activity Dashboard](https://chaszimmerman.github.io/git-sweaty/).  
...
A **Races** section appears below all year heatmaps, listing all detected race activities in reverse-chronological order.
...
- **Per-race medals** (🥇 / 🥈 / 🥉) appear to the left of a race name showing whether that was your 1st, 2nd, or 3rd fastest running of *that specific race* (ranked by pace across every year you've run it; only for races you've done 2+ times). This is separate from the PR badge — the medal compares you to your own history at that one race, while the PR badge compares against all your efforts at that distance
...
- **Multi-year progression chart**: races you've run in 2+ years (same race name) get a clickable row that expands an inline dual-axis chart — average pace (inverted, so getting faster trends upward) vs. average heart rate across every year you ran it. One chart open at a time; the chart always shows the full multi-year history regardless of the year filter
- **Race-day weather strip**: below the progression chart, a row of colored chips (one per year) shows the actual ambient temperature at each race's start time — cool (blue) → hot (red) — so you can see how conditions tracked with performance. Temperature is real weather from [Open-Meteo](https://open-meteo.com/) (free, no API key), looked up from each race's GPS start location and start hour, and cached in `data/race_weather.json`. A neutral `—` chip appears for a race with no GPS coordinates (e.g. treadmill). Hovering a chip shows a tooltip with the **feels-like** (humidity/wind-adjusted) temperature alongside the air temperature. Note: Strava's own device `average_temp` is intentionally **not** used — on wrist devices it reads body heat, not ambient air
- **Per-mile split bars**: stacked below the trend chart, a grouped bar chart compares your pace for each mile across years — one group per mile, one bar per year (oldest → newest, left → right), taller = faster. Sourced from Strava's per-mile splits (`splits_standard`), cached in `data/race_splits.json`. The final partial mile is included only when it's at least 0.75 mi (so a 5K shows Miles 1–3, a ~4-miler keeps its final 0.8 as Mile 4). Long races scroll horizontally
