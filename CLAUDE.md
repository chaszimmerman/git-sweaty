...
- Per-race medal (`race.medalRank`, 🥇🥈🥉): **distinct from the PR badge.** Ranks each running against your OTHER runnings of that same named race series (`raceSeriesKey`), where the PR badge ranks against all efforts at that distance. Computed in `buildRacesCard` after `seriesMap` is built: within each series with ≥2 runnings, sort by pace (`movingTime ÷ distMi`, ascending; ties by earlier date) and tag the fastest three. Ranks every running (no per-year dedup) across all time, so it's stable under the year-chip filter. Rendered as a medal emoji at the **left of the race name** (`.races-medal`, before the chevron) with a title tooltip ("Your 2nd-fastest of N … runnings") — visually opposite the PR pills on the right so the two indicators don't read as the same thing.
...
### Month filter (top filter bar)
Third filter control alongside Activities and Year — desktop order left → right: Activities · Year · Month · Reset All · mi/km units pill (units toggle moved from an absolutely-positioned corner pill into its own grid cell so it can't collide with Reset All). All three filter controls are identical in markup/size; behavior mirrors the year filter exactly (see `docs/filter-behavior-lock.md`): implicit-all default, draft-until-Done dropdown, Done compresses explicit-all → implicit-all, year-style plain "Clear" action button, Reset All integration. No persistence.
- **Semantics:** calendar months (Jan–Dec chips), 0-based `monthIndex` convention matching the rest of app.js. Selecting "Jul" shows July activity across all selected years. Fixed 12-month domain (`MONTH_INDICES`) — no "visible months" recomputation.
- **Threading:** `update()` computes `months`/`monthSet` (`null` when all 12 selected, so all-months is a true no-op) and passes them into `getFilteredActivities`, `getTypeYearTotals`/`getTypesYearTotals`, `combineYearAggregates` (+ new `filterAggregatesByMonths` for the single-type card path), `buildSummary`, `buildStatsOverview` (via `options.monthSet`), `buildMonthlyHeatmap` (via `monthIndices` — renders only selected months' columns, YoY badges still index absolutely into the prior year), `buildRacesCard`, and `buildCard`/`buildHeatmapArea` (via `options.selectedMonthSet`). `monthIndexFromDateKey(dateStr)` parses month from `YYYY-MM-DD` keys.
- **Year grids:** keep their full Jan–Dec shape; in-year days outside selected months get `.cell.month-muted` (empty background, `pointer-events: none`, no tooltip/race ring, and the Days Off metric never paints them). Stat totals and per-card days-off use `getElapsedDayCountForYearInMonths` so denominators only count elapsed days inside selected months.
- **Layout:** desktop `.header-top` is now a 10-column grid (1fr | filter/action ×3 pairs | reset | units | 1fr) with narrower width vars at 925–1155px and a `@media (min-width: 1156px)` override; the desktop block also sets `.filter-menu { min-width: 0 }` (base 190px min-width would overflow the narrower columns). Tablet/mobile blocks gain a 6th grid row (Months) with Reset/units moving to row 6.
- Scroll keys embed the month selection (`:m:all` / `:m:5,6`); year cards with no activity in the selected months drop out (same as empty years); races series/PR/medal logic is untouched by the month filter.
